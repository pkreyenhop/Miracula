/* UTF-8 codec used by the lexer and runtime character I/O paths.
   The decoder reports malformed byte sequences as fatal protocol errors. */
...
#define foo(a)                                                                                     \
  if ((a) == EOF)                                                                                  \
    ...
    sprintf(p, " %#x", (unsigned)(a)), p += strlen(p)
#define err(a)                                                                                     \
  {                                                                                                \
    *errs = '\0';                                                                                  \
    p = errs;                                                                                      \
...
/* returns a unicode value or EOF for end of input */
{
  ...
  if ((c0 & 0xe0) == 0xc0) { /* 2 bytes */
    ...
    if ((c1 & 0xc0) != 0x80)
      err2(c0, c1);
    return ((unicode)((c0 & 0x1f) << 6 | (c1 & 0x3f)));
  }
  if ((c0 & 0xf0) == 0xe0) { /* 3 bytes */
    if ((nextch(c1)) == EOF)
      err2(c0, c1);
    if ((c1 & 0xc0) != 0x80)
      err2(c0, c1);
    ...
    if ((c2 & 0xc0) != 0x80)
      err3(c0, c1, c2);
    return ((unicode)((c0 & 0xf) << 12 | (c1 & 0x3f) << 6 | (c2 & 0x3f)));
  }
  if ((c0 & 0xf8) == 0xf0) { /* 4 bytes */
    if ((nextch(c1)) == EOF)
      err2(c0, c1);
    if ((c1 & 0xc0) != 0x80)
      err2(c0, c1);
    ...
    if ((c3 & 0xc0) != 0x80)
      err4(c0, c1, c2, c3);
    return ((unicode)((c0 & 7) << 18 | (c1 & 0x3f) << 12 | (c2 & 0x3f) << 6 | (c3 & 0x3f)));
  }
  err(c0);
  return ((unicode)EOF); /* Never happens 'cos err() exits; silence compiler warning */
}
