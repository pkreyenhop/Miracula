...
pub fn build(b: *std.Build) void {
    ...
    const run_phase5_types = b.addSystemCommand(&.{ "python3", "scripts/phase5_types.py" });
    const test_phase5 = b.step("test-phase5", "Verify typed value vocabulary and compatibility ratchet");
    test_phase5.dependOn(&run_phase5_types.step);

    const run_phase6_architecture = b.addSystemCommand(&.{ "python3", "scripts/phase6_architecture.py" });
    const test_phase6 = b.step("test-phase6", "Verify acyclic Go package map and state ownership");
    ...
    const run_phase8_protocol = b.addSystemCommand(&.{ "python3", "scripts/phase8_x_protocol.py" });
    const test_phase8 = b.step("test-phase8", "Verify the portable .x protocol and canonical vectors");
    test_phase8.dependOn(&run_phase8_protocol.step);
    test_phase8.dependOn(&run_main_tests.step);

    const run_phase9_platform = b.addSystemCommand(&.{ "python3", "scripts/phase9_platform.py" });
    const test_phase9 = b.step("test-phase9", "Verify typed platform services and supported-target CI");
    test_phase9.dependOn(&run_phase9_platform.step);
    test_phase9.dependOn(&run_main_tests.step);

    const run_phase10_strings = b.addSystemCommand(&.{ "python3", "scripts/phase10_strings.py" });
    const test_phase10 = b.step("test-phase10", "Verify typed parsers and sentinel-string ownership ratchets");
    ...
    const run_phase12_determinism = b.addSystemCommand(&.{ "python3", "scripts/phase12_determinism.py" });
    const test_phase12 = b.step("test-phase12", "Verify deterministic fixtures, generation, and unordered iteration ownership");
