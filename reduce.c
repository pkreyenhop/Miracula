...
/* the value of the whole expression is a list of `messages' */
{
  ...
  while (tag[e] == CONS) {
    ...
    case Tofileb:
      ...
      break;
    case Closefile:
      closefile(tl(hd(e)) = reduce(tl(hd(e))));
      break;
    case Appendfile:
    ...
    case Appendfileb:
      ...
      break;
    case System:
      system(getstring(tl(hd(e)) = reduce(tl(hd(e))), "System"));
      break;
    case Exit: {
...
void getenv_error(char *a) {
  fprintf(stderr, "program error: getenv(%s): illegal characters in result string\n", a);
  outstats();
  exit(1);
}
...
#endif
  fprintf(stderr, "||");
  fprintf(stderr, "reductions = %lld, cells claimed = %lld, ", cycles, cellcount + claims);
