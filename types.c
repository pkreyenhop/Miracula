...
#include "data.h"
#include "big.h"
int R=NIL;  /* direct-and-indirect dependency graph */
int TABSTRS=NIL; /* list of abstype declarations */
int ND;  /* undefined names used in script */
int SBND; /* names specified but not defined (handled separately) */
int FBS; /* list of bindings caused by parameterised %include's */
int ATNAMES; /* global var set by abstr_check */
int NT=NIL;  /* undefined typenames used in script */
int TYPERRS;
int bnf_t=0;
int current_id=0,lastloc=0,lineptr=0;   /* used to locate type errors */
int showchain=NIL; /* links together all occurrences of special forms (show)
...
{ int s;
  extern int freeids,SYNERR,fnts;
  ATNAMES=TYPERRS=0;
...
tsort(g) /* topological sort - returns a list of the elements in the domain
  ...
  it depends on */
int g;  /* the structure of g is "graph2" see NOTES above */
{ int NP=NIL; /* NP is set of elements with no predecessor */
  int g1=g, r=NIL; /* r is result */
  g=NIL;
  while(g1!=NIL)
    { if(tl[hd[g1]]==NIL)NP=cons(hd[hd[g1]],NP);
      ...
      g1=tl[g1]; }
  while(NP!=NIL)
  { int D=NIL; /* ids to be removed from range of g */
    while(NP!=NIL)
    { r=cons(hd[NP],r);
...
int hereinc; /* location of currently-being-processed %include */
int lasthereinc;
...
int tabstrs;
{ while(tabstrs!=NIL)
     { int atnames=hd[hd[tabstrs]],sigids=tl[hd[tabstrs]],rtypes=NIL;
       if(cyclic_abstr(atnames))return;
       while(sigids!=NIL) /* compute representation types */
          { int rt=rep_t(id_type(hd[sigids]),atnames);
...
int x;
{ int rtypes=tl[hd[x]],sigids=tl[x];
/*int holdat=ATNAMES;
...
int T,L;
{ int args=NIL,t1,new=0;
  for(t1=T;iscompound_t(t1);t1=hd[t1])
     { int a=rep_t1(tl[t1],L);
       if(a!=tl[t1])new=1;
       args=cons(a,args); }
...
int tfnum,tfbool,tfbool2,tfnum2,tfstrstr,tfnumnum,ltchar,
    tstep,tstepuntil;
...
{ int bnftokenstate=findid("bnftokenstate");
...
{ int t=id_type(col_fn),f=tf(tl[hd[tl[bnf_t]]],num_t);
...
int x,env;   /* env is list of local bindings of variables to types */
int ngt;     /* ngt is list of non-generic type variables */
{ int a,b,c,d; /* initialise to 0 ? */
  switch(tag[x])
  { case AP: if(hd[x]==BADCASE||hd[x]==CONFERROR)return(NTV);
	     /* don't type check insides of error messages */
             { int ft=etype(hd[x],env,ngt),at=etype(tl[x],env,ngt),rt=NTV;
		   ...
		   return(NTV); }
	       return(rt); }
    case CONS: { int ht=etype(hd[x],env,ngt),rt=etype(tl[x],env,ngt);
    ...
    case ATOM: if(x<256)return(char_t);
	       ...
	       case EXEC: a=ap2(comma_t,ltchar,ap2(comma_t,num_t,void_t));
			  return(tf(ltchar,ap2(comma_t,ltchar,a)));
	       case READBIN:
	       case READ: return(tf(char_t,ltchar)); /* $- is ap(READ,0) */
	       case FILEMODE:
	       case GETENV:
	       case NB_STARTREAD:
	       case STARTREADBIN:
...
{ if(isvar_t(t))return((*f)(t));
  if(iscompound_t(t))
  { int h1=walktype(hd[t],f);
    int t1=walktype(tl[t],f);
...
int SUBST[hashsize];  /* hash table of substitutions */
...
int t;   /* relevant tvars are those not in ngt */
...
int t;  /* t MUST be in reduced form - see redtvars */
{ tvmap=NIL;
...
int s1,s2; /* both are in ascending address order */
{ int *ss1= &s1;
...
int allchars=0; /* flag used by tail */
