/* MIRANDA TYPECHECKER  */
/* Owns Miranda's type inference, type synonym expansion, abstract type checks,
   dependency analysis, and reporting for undefined or ill-typed names. */
 ...
 \"%s\"\n",
  ...
  if (id_type(n) != undef_t) /* meta typecheck spec, if present */
  {
    ...
    if (tag[id_type(n)] == CONS) { /* signature identifier */
      ...
      current_id = 0;
      return;
    } /* typechecked separately, under TABSTRS */
...
word tfnum, tfbool, tfbool2, tfnum2, tfstrstr, tfnumnum, ltchar, tstep, tstepuntil;
...
word genlstat_t(void) /* type of %lex state */
{
...
/* ngt is list of non-generic type variables */
{
  ...
  case TCONS:
    return (ap2(comma_t, etype(hd(x), env, ngt), etype(tl(x), env, ngt)));
  case PAIR:
    return (ap2(comma_t, etype(hd(x), env, ngt), ap2(comma_t, etype(tl(x), env, ngt), void_t)));
  case DOUBLE:
  case INT:
    return (num_t);
  case ID:
  ...
  case ATOM:
    ...
    default:
      printf("do not know type of ");
      out(stdout, x);
      putchar('\n');
      return (wrong_t);
    }
  default:
    printf("unexpected tag in etype ");
    out(stdout, tag[x]);
    putchar('\n');
    return (wrong_t);
  }
...
{
  locate("error");
  printf("illegal object \"");
  out_pattern(stdout, x);
...
void out_formal1(FILE *f, word x) {
  ...
  if (x == NIL)
    ...
    out_pattern(f, x);
    fprintf(f, ")");
  } else if (tag[x] == TCONS || tag[x] == PAIR) {
    fprintf(f, "(");
    while (tag[x] == TCONS) {
      out_pattern(f, hd(x));
      x = tl(x);
    ...
    out_pattern(f, tl(x));
    fprintf(f, ")");
  } else if ((tag[x] == INT && neg(x)) || (tag[x] == DOUBLE && get_dbl(x) < 0)) {
    fprintf(f, "(");
    out(f, x);
    fprintf(f, ")");
  } /* -ve numbers */
