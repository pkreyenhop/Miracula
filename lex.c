...
/* Owns source input, offside/layout handling, string and number tokenisation,
   dictionary storage for names, and file inclusion state. */
...
void setupdic(void) {
  ...
  prefixbase = (char *)malloc(prefixlimit);
  prefixbase[0] = '\0';
  prefix = 0;
}
...
void dicovflo(void) /* is this called everywhere it should be? Check later */
{
...
char *token(void) /* lex analyser for command language (very simple) */
{
  ...
  if (ch != '"' && ch != '<') /* test added 9.5.06 see else part */
    ...
    word closeq = ch == '<' ? '>' : '"'; /* this branch added 9.5.06          */
...
int yylex(void) /* called by YACC to get the next symbol */
{
...
word directive(void) /* these are of the form "%identifier" */
{
  ...
  case 'f':
    ...
    break;
  case 'i':
    if (is("include") || is("_i_n_c_l_u_d_e")) {
      if (!SYNERR) {
        layout();
        setlmargin();
      }
    ...
    if (is("insert") || is("_i_n_s_e_r_t")) {
      ...
      if (!f) {
        ...
        c = getch();
      } /* used to precede previous cmd when echo
  ...
  case 'l':
    ...
    break;
  case 'n':
    if (is("nolist") || is("_n_o_l_i_s_t")) {
      echoing = 0;
      return (yylex());
    }
    break;
...
void hexnumeral(void) /* added 21.11.2013 */
{
  dicq = dicp;
  *dicq++ = c, c = getch(); /* 0 */
  *dicq++ = c, c = getch(); /* x */
...
{
  while (x != NIL) { /* char *s = get_id(hd(x)); unused variable & get_id has no side effects */
    get_id(hd(x))[0] += 128; /* hack to make private internal name */
    x = tl(x);
  } /* NB - doesn't change hashbucket */
  inprelude = 0;
}
