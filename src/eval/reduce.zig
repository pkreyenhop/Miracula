...
const combinators_list = @import("combinators/combinators_list.zig");
    ...
    main_loop: while (true) {
        ...
        if (@as(u64, @bitCast(ctx.e.toRaw())) >= word.ATOMLIMIT) {
            ...
            switch (ctx.e.toRaw()) {
                ...
                word.MAP => try combinators_list.handleMAP(&ctx),
                word.FLATMAP => try combinators_list.handleFLATMAP(&ctx),
                word.FILTER => try combinators_list.handleFILTER(&ctx),
                word.LIST_LAST => try combinators_list.handleLIST_LAST(&ctx),
                word.LENGTH => try combinators_list.handleLENGTH(&ctx),
                word.DROP => try combinators_list.handleDROP(&ctx),
                word.SUBSCRIPT => try combinators_list.handleSUBSCRIPT(&ctx),
                word.FOLDL1 => try combinators_list.handleFOLDL1(&ctx),
                word.FOLDL => try combinators_list.handleFOLDL(&ctx),
                word.FOLDR => try combinators_list.handleFOLDR(&ctx),
