// Miranda runtime constants: cell tags (data.h), combinator codes (combs.h),
// parser token values (y.tab.h), and miscellaneous scalar limits.
//
// This file is the single Zig source of truth for constants that were
// previously spread across the C headers data.h, combs.h, and y.tab.h.
// Import as:  const w = @import("path/to/word.zig");

/// The Miranda heap word type.  Identical to C's `typedef long word` on both
/// 32-bit and 64-bit platforms.
pub const Word = c_long;

// ── Cell tag values ──────────────────────────────────────────────────────────
// Stored in tag[cell_index].  Values 0..TCONS are valid tags.

pub const ATOM: u8 = 0;
pub const DOUBLE: u8 = 1;
pub const DATAPAIR: u8 = 2;
pub const FILEINFO: u8 = 3;
pub const TVAR: u8 = 4;
pub const INT: u8 = 5;
pub const CONSTRUCTOR: u8 = 6;
pub const STRCONS: u8 = 7;
pub const ID: u8 = 8;
pub const AP: u8 = 9;
pub const LAMBDA: u8 = 10;
pub const CONS: u8 = 11;
pub const TRIES: u8 = 12;
pub const LABEL: u8 = 13;
pub const SHOW: u8 = 14;
pub const STARTREADVALS: u8 = 15;
pub const LET: u8 = 16;
pub const LETREC: u8 = 17;
pub const SHARE: u8 = 18;
pub const LEXER: u8 = 19;
pub const PAIR: u8 = 20;
pub const UNICODE: u8 = 21;
pub const TCONS: u8 = 22;

// ── Parser token values (formerly y.tab.h) ───────────────────────────────────
// Tokens in the range 257..CMBASE-1; CMBASE itself is the first combinator.
...
pub const FREE: Word = 276; // lexer token (not the type-class constant free_t)
...
// ── Combinator codes (formerly combs.h) ──────────────────────────────────────
// All combinator and special-value atoms live in the range CMBASE..ATOMLIMIT-1.
...
pub const GETENV_FN: Word = CMBASE + 75; // named GETENV_FN; GETENV alone clashes with nothing but be explicit
...
// ── Miranda type constants (data.h) ──────────────────────────────────────────
// Used by the type-checker; live as small integer values, not heap pointers.
...
// Type class values (t_class field of a type identifier's value cell).
pub const algebraic_t: Word = 0;
pub const synonym_t: Word = 1;
pub const abstract_t: Word = 2;
pub const placeholder_t: Word = 3;
pub const free_t: Word = 4; // distinct from the FREE lexer token (276)

// ── Miscellaneous scalar constants (data.h) ───────────────────────────────────

pub const XVERSION: Word = 83; // dump-format version; bump on incompatible changes
pub const BUFSIZE: Word = 1024; // shell command length limit
pub const pnlim: Word = 1024; // pathname length limit
pub const hashsize: Word = 512; // type-checker unification hash table size
pub const YYMAXDEPTH: Word = 1000;

// Sentinel / tag values used in grammar / evaluation
pub const END: Word = 0;
pub const GENERATOR: Word = 0;
pub const GUARD: Word = 1;
pub const REPEAT: Word = 2;

// ── Heap pointer bit-field masks (data.h) ────────────────────────────────────
// The tl[] field uses the top two bits as GC pointer flags on 64-bit.

pub const wordsize: comptime_int = @sizeOf(Word) * 8;
pub const BACKSTOP: Word = @bitCast(@as(usize, 1) << (wordsize - 1));
pub const tlptrbit: Word = BACKSTOP;
pub const tlptrbits: Word = @bitCast(@as(usize, 3) << (wordsize - 2));

// ── Dump-format type codes (data.h: XBASE = ATOMLIMIT - 256) ─────────────────

pub const XBASE: Word = ATOMLIMIT - 256;
pub const CHAR_X: Word = XBASE + 0;
...
pub const XLIMIT: Word = XBASE + 16;
