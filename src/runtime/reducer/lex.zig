...
pub fn handle_LEX_TRY1_(ctx: *ReductionCtx) void {
    ...
    while (true) {
        ...
        const tl_hd_hd_arg1 = reduce.tl_get(reduce.hd_get(reduce.hd_get(ctx.args[0])));
        reduce.rewrite_to_cons(ctx.e, reduce.ap2(reduce.tl_get(reduce.tl_get(reduce.hd_get(ctx.args[0]))), clib.lexstate(lastarg(ctx)), reduce.ap(word.DESTREV, reduce.hd_get(ctx.hold))), reduce.cons(if (tl_hd_hd_arg1 != 0) tl_hd_hd_arg1 - 1 else ctx.args[1], reduce.tl_get(ctx.hold)));
        ctx.action = word.ACT_DONE;
        return;
    }
}
...
pub fn handle_LEX_COUNT(ctx: *ReductionCtx) void {
    ...
    reduce.rewrite_to_cons(ctx.e, clib.strcons(reduce.hd_get(ctx.args[0]), ctx.hold), reduce.ap(word.LEX_COUNT, ctx.args[0]));
    if (ctx.hold == '\n') {
