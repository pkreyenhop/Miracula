...
export fn handle_G_ERROR(ctx: *ReductionCtx) void {
    ...
    if (ctx.hold != clib.NIL) {
        reduce.rewrite_to_value(&ctx.e, ctx.hold);
        ctx.action = clib.ACT_DONE;
        return;
    }
    ctx.hold = clib.g_residue(lastarg(ctx));
    reduce.rewrite_to_cons(ctx.e, reduce.ap(ctx.args[1], ctx.hold), clib.NIL);
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_G_OPT(ctx: *ReductionCtx) void {
    ...
    ctx.hold = reduce.reduce(ctx.hold);
    if (ctx.hold == clib.NIL) {
        reduce.rewrite_to_cons(ctx.e, clib.NIL, lastarg(ctx));
    } else {
        reduce.rewrite_to_cons(ctx.e, reduce.cons(reduce.hd_get(ctx.hold), clib.NIL), reduce.tl_get(ctx.hold));
    }
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_G_STAR(ctx: *ReductionCtx) void {
    ...
    tag[reduce.clean_ptr(ctx.e)] = clib.CONS;
    reduce.hd_set(ctx.e, reduce.cons(reduce.hd_get(ctx.hold), reduce.ap(clib.HD, ctx.args[1])));
    reduce.tl_set(ctx.e, reduce.ap(clib.TL, ctx.args[1]));
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_G_FBSTAR(ctx: *ReductionCtx) void {
    ...
    ctx.hold = reduce.reduce(ctx.hold);
    if (ctx.hold == clib.NIL) {
        reduce.rewrite_to_cons(ctx.e, clib.I, lastarg(ctx));
        ctx.action = clib.ACT_DONE;
        return;
    }
    reduce.hd_set(ctx.e, reduce.ap2(clib.G_SEQ, reduce.hd_get(ctx.e), reduce.ap(clib.G_RULE, reduce.ap(clib.CB, reduce.hd_get(ctx.hold)))));
...
export fn handle_G_SYMB(ctx: *ReductionCtx) void {
    ...
    if (lastarg_reduced == clib.NIL) {
        reduce.rewrite_to_nil(&ctx.e);
        ctx.action = clib.ACT_DONE;
        return;
    }
    reduce.hd_set(lastarg_reduced, reduce.reduce(reduce.hd_get(lastarg_reduced)));
    ctx.hold = reduce.ap(clib.HD, reduce.hd_get(lastarg_reduced));
    if (clib.compare(ctx.args[0], reduce.reduce(ctx.hold)) != 0) {
        reduce.rewrite_to_value(&ctx.e, clib.NIL); // rewrite_to_failure in C is NIL
    } else {
...
export fn handle_G_ANY(ctx: *ReductionCtx) void {
    ...
    set_lastarg(ctx, lastarg_reduced);
    if (lastarg_reduced == clib.NIL) {
        reduce.rewrite_to_value(&ctx.e, clib.NIL);
    } else {
        reduce.rewrite_to_cons(ctx.e, reduce.ap(clib.HD, reduce.hd_get(lastarg_reduced)), reduce.tl_get(lastarg_reduced));
    }
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_G_SUCHTHAT(ctx: *ReductionCtx) void {
    ...
    ctx.hold = reduce.ap(clib.HD, reduce.hd_get(lastarg_reduced));
    ctx.hold = reduce.reduce(ctx.hold);
    if (reduce.reduce(reduce.ap(ctx.args[0], ctx.hold)) == clib.True) {
...
export fn handle_G_END(ctx: *ReductionCtx) void {
    ...
    set_lastarg(ctx, lastarg_reduced);
    if (lastarg_reduced == clib.NIL) {
        reduce.rewrite_to_cons(ctx.e, clib.NIL, clib.NIL);
    } else {
        reduce.rewrite_to_value(&ctx.e, clib.NIL);
    }
    ctx.action = clib.ACT_DONE;
...
export fn handle_G_STATE(ctx: *ReductionCtx) void {
    ...
    if (lastarg_reduced == clib.NIL) {
        ...
        reduce.rewrite_to_cons(ctx.e, reduce.ap(clib.TL, reduce.hd_get(lastarg_reduced)), lastarg_reduced);
    }
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_G_UNIT(ctx: *ReductionCtx) void {
    ...
    reduce.rewrite_to_cons_head(ctx.e, clib.I);
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_G_CLOSE(ctx: *ReductionCtx) void {
    ...
    ctx.args[2] = reduce.ap(clib.G_COUNT, lastarg(ctx));
    ctx.hold = reduce.ap(ctx.args[1], ctx.args[2]);
    ctx.hold = reduce.reduce(ctx.hold);
    if (ctx.hold == clib.NIL) {
        clib.reduce_parse_close_error(ctx.args[0], ctx.args[2]);
    }
...
export fn handle_G_COUNT(ctx: *ReductionCtx) void {
    ...
    reduce.rewrite_to_cons(ctx.e, reduce.hd_get(lastarg_reduced), reduce.ap(clib.G_COUNT, reduce.tl_get(lastarg_reduced)));
    ctx.action = clib.ACT_DONE;
}
...
export fn handle_LEX_RPT1(ctx: *ReductionCtx) void {
    ...
    reduce.tl_set(ctx.e, clib.LEX_COUNT0);
    reduce.downLeft(ctx);
    reduce.downLeft(ctx);
    ctx.action = clib.ACT_NEXTREDEX;
}
...
export fn handle_LEX_TRY(ctx: *ReductionCtx) void {
    ...
    reduce.hd_set(ctx.e, clib.LEX_TRY_);
    reduce.downLeft(ctx);
    ctx.action = clib.ACT_NEXTREDEX;
}
...
export fn handle_LEX_TRY_(ctx: *ReductionCtx) void {
    ...
    while (true) {
        if (ctx.args[0] == clib.NIL) {
            clib.lexfail(lastarg(ctx));
        }
        const hd_hd_hd_arg1 = reduce.hd_get(reduce.hd_get(reduce.hd_get(ctx.args[0])));
        if (hd_hd_hd_arg1 != 0 and clib.member(hd_hd_hd_arg1, ctx.args[1]) == 0) {
...
export fn handle_LEX_TRY1(ctx: *ReductionCtx) void {
    ...
    reduce.tl_set(ctx.e, reduce.reduce(reduce.tl_get(ctx.e)));
    reduce.force(reduce.tl_get(ctx.e));
    reduce.hd_set(ctx.e, clib.LEX_TRY1_);
    reduce.downLeft(ctx);
    ctx.action = clib.ACT_NEXTREDEX;
}
...
export fn handle_LEX_COUNT0(ctx: *ReductionCtx) void {
    ...
    reduce.hd_set(ctx.e, clib.LEX_COUNT);
    reduce.tl_set(ctx.e, clib.strcons(0, reduce.tl_get(ctx.e)));
    reduce.downLeft(ctx);
    ctx.action = clib.ACT_NEXTREDEX;
}
...
export fn handle_LEX_COUNT(ctx: *ReductionCtx) void {
    ...
    reduce.rewrite_to_cons(ctx.e, clib.strcons(reduce.hd_get(ctx.args[0]), ctx.hold), reduce.ap(clib.LEX_COUNT, ctx.args[0]));
    if (ctx.hold == '\n') {
...
export fn handle_LEX_STRING(ctx: *ReductionCtx) void {
    ...
    while (ctx.args[0] != clib.NIL) {
        const lastarg_reduced = reduce.reduce(lastarg(ctx));
        set_lastarg(ctx, lastarg_reduced);
        if (lastarg_reduced == clib.NIL or lh(lastarg_reduced) != reduce.hd_get(ctx.args[0])) {
            reduce.rewrite_to_nil(&ctx.e);
            ctx.action = clib.ACT_DONE;
            return;
        }
...
export fn handle_LEX_CHAR(ctx: *ReductionCtx) void {
    ...
    const lastarg_reduced = reduce.reduce(lastarg(ctx));
    set_lastarg(ctx, lastarg_reduced);
    if (lastarg_reduced == clib.NIL or lh(lastarg_reduced) != ctx.args[0]) {
        reduce.rewrite_to_nil(&ctx.e);
        ctx.action = clib.ACT_DONE;
        return;
    }
