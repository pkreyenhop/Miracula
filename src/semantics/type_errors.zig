...
//! docs/GoReady.md) — type-error reporting (`typeError`/
...
pub fn sayhere(heap: *Heap, h_val: Word, nl: Word) void {
    ...
    const eq = std.mem.eql(u8, h_str, script_store.store().current_script.?);
            ...
            type_t => {
                ...
                switch (getTag(heap, t_val)) {
                    ...
                    .STRCONS => {
                        ...
                        } else if (std.mem.eql(u8, strtab.strOf(strtab.table(), h(heap, t(heap, tInfo(heap, t_val)))), script_store.store().current_script.?)) {
