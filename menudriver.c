...
/* Standalone browser for the installed Miranda manual tree. */
...
static void menudrive(const char *);
static void singleton(const char *);
...
void menudrive(const char *dir) {
  ...
  while (stat("contents", &buf) == 0) {
    ...
    } else if (strcmp(next, "???") == 0) /* ask to see menudriver settings */
    {
      ...
      exit(0);
    else if (next[0] == '!') /* shell escape - handy for editing manual! */
    {
